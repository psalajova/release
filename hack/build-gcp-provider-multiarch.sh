...
# Pushes to both quay.io/openshift/ci and quay.io/openshift/ci-public.
...
TARGET_CI="quay.io/openshift/ci:${IMAGE_TAG}"
TARGET_CI_PUBLIC="quay.io/openshift/ci-public:${IMAGE_TAG}"

echo "Version:       ${VERSION}"
echo "Source image:   ${UPSTREAM_IMAGE}:${VERSION}"
echo "Target images:  ${TARGET_CI}"
echo "                ${TARGET_CI_PUBLIC}"
echo
...
${CONTAINER_ENGINE} manifest create "${TARGET_CI}"
...
")
    ...
    ${CONTAINER_ENGINE} manifest add "${TARGET_CI}" \
...
${CONTAINER_ENGINE} manifest inspect "${TARGET_CI}" | \
    python3 -c "
import sys, json
data = json.load(sys.stdin)
for m in data['manifests']:
    ...
    if os == 'unknown':
        continue
    print(f'  {os}/{arch}: {m[\"digest\"][:20]}...')
"

echo
echo "Pushing to quay.io/openshift/ci..."
${CONTAINER_ENGINE} manifest push "${TARGET_CI}"

echo "Pushing to quay.io/openshift/ci-public..."
${CONTAINER_ENGINE} manifest push "${TARGET_CI}" "docker://${TARGET_CI_PUBLIC}"
